...
| Scholarship | CMS scholarship-cycle record plus approved Wix Form/CRM workflow and Media documents | Show current status, eligibility, requirements, accessible form handoff/shell | Confirm API support, sensitive-data handling, upload/signature accessibility, retention/privacy, closed state; provide email/mail fallback only if approved |
...
| `/ad-booklet` | Current canonical event/campaign destination `[NEEDS CLIENT INPUT: Confirm whether the ad-booklet campaign remains active]` | Never redirect to an expired or unrelated offer; use `/events/` if no equivalent survives |
